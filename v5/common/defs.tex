...
%-----------------------------------------------------------------------------------------
\NewDocumentCommand{\myReverseBox}{mm O{2} O{\text{\:}}}{
    \small
    \mycirc{1}\underline{\myEmph{Rewrite:}} \fbox{ \phantom{$ax^m + bx^n + c$} }\\[0.25\onelineskip]
    \mycirc{2}\underline{\myEmph{Constants:}} 
    \begin{tabular}{llll}
        $a = $ & \fbox{\phantom{999}}\\
        $b = $ & \fbox{\phantom{999}}& $a\cdot c = $ & \fbox{\phantom{999}}\\ 
        $c = $ & \fbox{\phantom{999}}\\
    \end{tabular}\\[0.25\onelineskip]
    \mycirc{3}\underline{\myEmph{T-chart:}}\\[0.1\onelineskip]
    \begin{tabular}{c|c}
        factors of \fbox{\phantom{999}} & sum (look for \fbox{\phantom{999}}~) \\
        \hline\hline 
        & \\ 
        & \\
        & \\
        & \\
    \end{tabular}\\[0.25\onelineskip]
    \mycirc{4}\mycirc{5}\underline{\myEmph{reverse box:}}\\[0.1\onelineskip]
    {
        \renewcommand{\arraystretch}{1.25}
    \begin{tabular}{r||r|r|}
        \cellcolor{lightgray}&&\\
        \hline\hline
        \phantom{GCF~~~}& \phantom{999999}$x^#3$& \phantom{99999}$x^#4$\\
        \hline
                         & $x^#4$             & \\
        \hline
    \end{tabular}
...
% #3 : variable symbol
%-----------------------------------------------------------------------------------------
\NewDocumentCommand{\myFourTermCubicReverseBox}{mm O{x}}{
    \small
    \mycirc{1}\underline{\myEmph{Descending:}} \fbox{ \phantom{$ax^m + bx^n + c$} }\\[0.25\onelineskip]
    \mycirc{2}\underline{\myEmph{Constants:}} 
    \begin{tabular}{llll}
        $a = $ & \fbox{\phantom{999}} & $c = $ & \fbox{\phantom{999}}\\
        $b = $ & \fbox{\phantom{999}}&  $d = $ & \fbox{\phantom{999}}\\ 
    \end{tabular}\\[0.25\onelineskip]

    \mycirc{3}\mycirc{5}\underline{\myEmph{reverse box:}}\\[0.1\onelineskip]
    {
        \renewcommand{\arraystretch}{1.25}
    \begin{tabular}{r||r|r|}
        \cellcolor{lightgray}&&\\
        \hline\hline
        \phantom{GCF~~~}& \phantom{999999}$#3^3$& \phantom{99999}$#3^2$\\
        \hline
                         & $#3$             & \\
        \hline
    \end{tabular}
